...
def remove_crashes(grid, carts)
  ...
  carts.reject!{|c| crash_spots.include?("#{c.x},#{c.y}")}
end

$ticks = 0
while true
  # print_world(grid, carts)
  last_count = carts.length
  carts
    .sort_by{|c| [c.y, c.x] }
    .map {|c| c.tick!(grid); remove_crashes(grid, carts); }

  if last_count != carts.length
    puts "Crash! Carts removed!"
  end
